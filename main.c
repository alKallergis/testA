...
//TODO://changε minUnsmoothed1 to<1400,add MAP_UtilsDelay(MILLISECONDS_TO_TICKS(200)) in line 1052, because first wave settling used to be problematic.
// Standard includes
#include <string.h>
#include <stdlib.h>
#include <math.h>
...
#define maxSweepFCount     4100 //TODO:4100 max measurements for now
...
int minsmoothedTimestamp1,minsmoothedTimestamp0,mincounter0,mincounter1;
float stampDiff;
static unsigned short D,Dprev,Dinitial;
...
void changeGain(unsigned char d){//d=0-7
    gain=1<<d;
...
void findInitialGain(){
    ...
    changeGain(D);
    Dinitial=D;//remember the initial gain. TODO: remove this on final accurate version. will recalculate in each sweep.
}
...
void smoothenAndEvaluate(){
    ...
    //smoothingInterval=3;
    smoothingInterval=G*log(2*4000/freq);//NOTE changed endFreq here to 4000 since it works, permanently
    ...
    if(mode!=3){//no use detecting input channel edges for square waveform
        i7=minsmoothedTimestamp0-minsmoothedTimestamp1;
        i8=minsmoothedTimestamp0-(minsmoothedTimestamp1+((float)adcIndex1/periodsToScan));//(from notes)add 1 period in case the 1st edge didn't get picked at the dds source signal,causing it to show 1 period later.
        i9=minsmoothedTimestamp1-(minsmoothedTimestamp0+((float)adcIndex1/periodsToScan));
...
void finalSmoothingImpedance(){
    //smoothen out impedance
        unsigned char smoothingInterval1=11;//todo:test this
        char i3;
        int i1,i2;
...
void setupSweep(){
        TimerEnable(TIMERA2_BASE,TIMER_BOTH);//todo: ENABLE THE AD 9833 CLOCK HERE.INTERFERES WITH WIFI!
...
int cellServer(unsigned short usPort)
{
    ...
    setupSweep();//set the sweep's parameters in here.//TODO: IN FINAL ACCURATE VERSION WE WILL RE-CALCULATE IN EACH SWEEP!
    ...
    while(1){
            ...
            while(TA3running==true);
            //delay for the last 30s...(If measurement takes longer because step is small,then we will delay more than the requested interval.)
            TA3running=true;
            TimerLoadSet(TIMERA3_BASE,TIMER_A,MILLISECONDS_TO_TICKS(1000.0*30));
            TimerEnable(TIMERA3_BASE,TIMER_A);
